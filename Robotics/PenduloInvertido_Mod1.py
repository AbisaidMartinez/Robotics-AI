...
def f(t, x):  # define la ec. dif. a resolver
    ...
    x4 = x[3]
    x_1p = x2
    x_2p = ((ff+mp*np.sin(x3)*(l*pow(x4, 2)+g*np.cos(x3))) /
            ((mc1+mc2)+mp*pow(np.sin(x3), 2)))
    # x_2p=(1/(mc+mp*pow(np.sin(x3),2)))*(ff+mp*np.sin(x3)*(l*pow(x4,2)+g*np.cos(x3)));
    x_3p = x4
    x_4p = (-(((ff+mp*l*pow(x4, 2))*np.cos(x3)+(mc1+mc2+mp)*(g))
            * np.sin(x3))/(l*((mc1+mc2)+mp*pow(np.sin(x3), 2))))
...
rect2 = ax.add_patch(
    Rectangle(xy=(x10, 0), width=1.5, height=0.5, color='orange'))
...
line1, = ax.plot([x10+1.5, 0.5], [x30+4.1, 0.5], lw=1.5, c='white',
                 path_effects=[pe.Stroke(linewidth=5, foreground='k'), pe.Normal()])
rect3 = ax.add_patch(
    Rectangle(xy=(x30+4.1, 0), width=3, height=1.5, color='blue'))
rect4 = ax.add_patch(
    Rectangle(xy=(x30+5.6, 1), width=1.5, height=0.5, color='orange'))
circle4 = ax.add_patch(plt.Circle((x30+5, 0), 0.2, color='k'))
circle5 = ax.add_patch(plt.Circle((x30+9.3, 0), 0.2, color='k'))
ax.set_xlim([-10, 20])
ax.set_ylim([-5, 5])
...
def animate(i):
    ...
    circle1.set_center((1.5+x2p[i], y2p[i]))
    # Actualiza las coord. cada frame para el centro del circulo
    circle2.set_center((x1[i]+0.5, 0))
    # Actualiza las coord. cada frame para el centro del circulo
    circle3.set_center((x1[i]+2.5, 0))
    circle4.set_center((x3p[i]+4.6, 0))
    circle5.set_center((x3p[i]+6.5, 0))
